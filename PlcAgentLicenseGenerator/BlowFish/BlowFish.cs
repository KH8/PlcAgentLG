...
namespace PlcAgentLicenseGenerator.BlowFish
{
    class BlowFish
    {
        ...
        private void SetupKey(byte[] cipherKey)
        {
            ...
            for (int i = 0; i < 18; i++)
            {
                var d = (uint)(((_key[j % cipherKey.Length] * 256 + _key[(j + 1) % cipherKey.Length]) * 256 + _key[(j + 2) % cipherKey.Length]) * 256 + _key[(j + 3) % cipherKey.Length]);
                _bfP[i] ^= d;
